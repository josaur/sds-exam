...
source("~/VSC/sds-exam/exam_causal_inference/replication/functionDefs.R")

sessionInfo()


dat <- read.dta13("~/VSC/sds-exam/exam_causal_inference/replication/CountryEPData.dta", convert.factors=F)
dat <- subset(dat, subset=dat$ccode %nin% c(5,6,9,12))
dat <- subset(dat, subset=dat$ep_election!=2019)

datIP <- read.dta13("~/VSC/sds-exam/exam_causal_inference/replication/CountryEPData_covIP.dta")
